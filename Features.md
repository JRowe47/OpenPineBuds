...
- **Application path:** The interpolated, smoothed curve forms the base compensation layer feeding the IIR/biquad EQ fitter per ear; the fitter honors section budgets per ear and falls back to a smoothed baseline when a requested filter set cannot be realized. The same base curve modulates all listening modes (ambient, music, theater, conversation, calls, tinnitus masking) after per-bud calibration, with mode-specific EQ applied as an overlay.
