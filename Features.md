...
- **Backward compatibility:** Standard octave-only audiograms remain valid and yield equivalent results after interpolation. Legacy configs continue to load using the prior default (`audiogram/v2` when unspecified), and a documented migration path to `audiogram/v3` keeps outputs within tolerance while enabling arbitrary point lists.
